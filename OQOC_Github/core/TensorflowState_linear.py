...
class TensorflowState:
    ...
    def init_defined_functions(self):
        ...
        if self.sys_para.traj:
            ...
            if self.sys_para.forbid:
                self.forbid_op = tf.constant(self.sys_para.forbid_op)
        ...
        if self.sys_para.state_transfer == False:
            ...
            if self.sys_para.expect:
                if self.sys_para.do_all:
                    ...
                    self.loss = - (self.Il*self.Il)
                    self.quad = tf.gradients(self.loss, [self.ops_weight_base])[0]
                    
                    if self.sys_para.forbid:
                        self.loss = self.loss
                        self.loss = self.loss + 0.01*tf.square(tf.reduce_sum(tf.multiply((tf.add(self.forbids_stacked[0,:,0] , self.forbids_stacked[1,:,0])),self.accelerate)))
                        self.loss = self.loss + 10000*(1-self.get_inner_product_2D(self.final_state,self.packed_initial_vectors))
                else: 
    ...
    def All_Trajectories(self,psi0):
        ...
        for ii in np.arange(0,self.sys_para.steps):
            ...
            if self.sys_para.forbid:
                ...
                self.forbids.append(self.expect(self.forbid_op, self.new_psi))
                self.all_forbids.append(self.expect2(self.forbid_op, self.new_psi))
        ...
        if self.sys_para.expect:
            if self.sys_para.do_all:
                if self.sys_para.forbid:
                    self.forbids_stacked = tf.stack(self.forbids,axis=1)
                    self.all_forbids = tf.stack(self.all_forbids,axis =1)
