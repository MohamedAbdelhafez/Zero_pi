    ...
    def plot_summary(self):
        ...
        if self.anly.expects != []:
            if self.sys_para.do_all:
                plt.subplot(gs[m, :],title="Expectation")
        ...
        if self.anly.forbids != []:
            if self.sys_para.do_all:
                plt.subplot(gs[-1, :],title="Cavity Occupartion")
                plt.plot(np.array([self.sys_para.dt* ii for ii in range(self.sys_para.steps)]),np.array(self.anly.forbids[0,:,0]),label="Real_g")
                
                plt.plot(np.array([self.sys_para.dt* ii for ii in range(self.sys_para.steps)]),np.array(self.anly.forbids[1,:,0]),label="Real_e")
