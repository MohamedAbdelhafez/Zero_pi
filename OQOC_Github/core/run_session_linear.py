    ...
    def start_adam_optimizer(self):
        ...
        while True:
            ...
            if self.sys_para.traj and self.sys_para.expect and self.do_all:
                ...
                self.rl,self.grad_pack, self.norms,self.l,inter_vecs,all_norms, expects,expects2,forbids,forbids2 = self.session.run(
                [self.tfs.reg_loss,self.tfs.grad_pack,self.tfs.norms,self.tfs.loss,self.tfs.inter_vecs,self.tfs.all_norms, self.tfs.expectations, self.tfs.all_expectations,self.tfs.forbids_stacked, self.tfs.all_forbids ], feed_dict=self.feed_dict)
        ...
        if not self.end:
            ...
            if (self.iterations % self.conv.update_step == 0):
                ...
                self.anly = Analysis(self.sys_para, self.tfs.final_state, self.tfs.ops_weight, self.tfs.unitary_scale,
                                     self.tfs.inter_vecs,self.feed_dict, avg_inter_vecs = inters, expects = self.expects, forbids = self.forbids)
                self.save_data()
                self.display()
