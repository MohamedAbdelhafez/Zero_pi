...
class TensorflowState:
    ...
    def get_inner_product_2D(self,psi1,psi2):
        ...
        with tf.name_scope('inner_product'):
            ...
            ac_bd = tf.reduce_sum(tf.add(ac,bd))
            bc_ad = tf.reduce_sum(tf.subtract(bc,ad))
            reals = tf.square(ac_bd) # first trace inner product of all vectors, then squared
            imags = tf.square(bc_ad)
