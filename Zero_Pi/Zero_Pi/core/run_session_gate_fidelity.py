    ...
    def start_adam_optimizer(self):
        ...
        while True:
            ...
            if self.sys_para.traj and not self.do_all:
                ...
                max_traj = int(40000000/len(self.sys_para.H0))
                #max_traj = 1
                ...
                [self.tfs.grad_pack,self.tfs.norms,self.tfs.loss,self.tfs.reg_loss, self.tfs.inter_vecs,self.tfs.all_norms], feed_dict=self.feed_dict)
                    ...
                    self.grad_nj.append(self.grad_pack)
                    self.ls.append(self.loss)
                    self.rls.append(self.rloss)
                    #if kk ==0:
                ...
                if jump_traj == 0:
                    ...
                    needed_traj = np.max(needed_traj)*np.ones([num_psi0])
                
                jump_traj = np.sum(needed_traj)
                #if jump_traj > 0:
                    
                #self.divided_branches = self.divide(needed_traj,max_traj)
                self.divided_branches = [needed_traj]
